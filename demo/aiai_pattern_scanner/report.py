...
from template import build_templates, detect_launch_day
...
def build_report(csv_path: str, top_n: int = 12, out_html: str = None):
    ...
    for _, row in top.iterrows():
        ...
        # 候选股的"现在"对齐模板的"启动日"位置：末尾对齐到 post 段起点
        offset = -len(tmpl_pre) - (len(tmpl_pre) + len(tmpl_post) - len(seg))
        # 简化：右对齐（末根K线对应模板末尾）
...
if __name__ == "__main__":
    import sys, glob
    csv_path = sys.argv[1] if len(sys.argv) > 1 else sorted(glob.glob("aiai_pattern_scan_*.csv"))[-1]
