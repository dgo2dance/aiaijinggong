...
用法:
  python3 scanner.py                                    # 全 A 扫描（默认）
  python3 scanner.py --pool csv --limit 7               # 演示池
  python3 scanner.py --limit 300 --workers 12           # 快速测试
  python3 scanner.py --min-mktcap 5 --max-mktcap 30     # 小市值范围
  python3 scanner.py --exclude-bj                       # 排除北交所（默认开）
"""
...
from data_source import get_kline, load_stock_pool
...
def scan(pool: pd.DataFrame, templates: dict, limit: int = 0,
    ...
    def work(code: str):
        # 深市代码补齐前导零（如 CSV 写成 1234 实际是 001234）
        code_z = code.zfill(6)
...
def main():
    ...
    ap.add_argument("--pool", default="sina",
                    choices=["sina", "csv", "seed"],
                    help="股票池来源：sina=全A在线 | csv=本地stock_pool.csv | seed=内置种子")
    ap.add_argument("--limit", type=int, default=0, help="扫描数量(0=全部)")
    ap.add_argument("--min-mktcap", type=float, default=10, help="最小流通市值(亿)")
    ap.add_argument("--max-mktcap", type=float, default=0, help="最大流通市值(亿)，0=不限")
    ...
    ap.add_argument("--pool-file", default=None,
                    help="--pool csv 时的自定义 CSV 路径（默认 examples/stock_pool_demo.csv）")
    args = ap.parse_args()
    ...
    if args.pool == "csv" and args.pool_file:
        from data_source import load_stock_pool as _lsp
