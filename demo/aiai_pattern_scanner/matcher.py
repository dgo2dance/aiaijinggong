...
匹配逻辑：
- pre_score  (蓄势相似度): 候选股最近N天 vs 模板A → 高分 = 正在蓄势(启动前)
- post_score (拉升相似度): 候选股最近M天 vs 模板B → 高分 = 刚启动(拉升初期)
...
def vol_pattern(vol_norm: np.ndarray, pre_frac: float = 0.6) -> float:
    """
    量能形态评分: 前段缩量、后段温和放量得高分
    pre_frac: 前段占比例（蓄势期占比）
    """
    n = len(vol_norm)
    k = int(n * pre_frac)
    if k == 0 or n - k == 0:
...
def match_stock(df: pd.DataFrame, templates: dict) -> Optional[dict]:
    """
    一只候选股的双模板匹配
    分类规则（形状门槛 shape>=0.35 为硬条件）：
      - 启动前(蓄势中): pre_score>=55 且蓄势段形状过硬
      - 刚启动(拉升初期): post_score>=55 且拉升段形状过硬 且近5日不深回撤
      - 形态接近(观察): max>=48
    """
